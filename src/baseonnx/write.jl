...
writeproto(io::IO, args...) = writeproto(ProtoBuf.ProtoEncoder(io), args...)
writeproto(args...) = ProtoBuf.encode(args...)

enumordinal(x) = Integer(x)

# TODO: User supplied elemtype??
ValueInfoProto(name::String, shape, elemtype=Float32) =
ValueInfoProto(
    name=name,
    var"#type"=TypeProto(
...
tsp_d(s::String) = TensorShapeProto_Dimension(value=ProtoBuf.OneOf(:dim_param, s))
tsp_d(s::Symbol) = tsp_d(string(s))

tp_tensor_elemtype(i::Integer) = i
tp_tensor_elemtype(::Missing) = enumordinal(TensorProto_DataType.UNDEFINED)
tp_tensor_elemtype(::Type{Float32}) = enumordinal(TensorProto_DataType.FLOAT)
...
TensorProto(t::AbstractArray{Float64,N}, name ="") where N = TensorProto(
    ...
    data_type=enumordinal(TensorProto_DataType.DOUBLE),
    double_data = reshape(t,:),
    name=name)

TensorProto(t::AbstractArray{Float32,N}, name ="") where N = TensorProto(
    dims=collect(reverse(size(t))),
    data_type=enumordinal(TensorProto_DataType.FLOAT),
...
TensorProto(t::AbstractArray{Int64,N}, name ="") where N = TensorProto(
    dims=collect(reverse(size(t))),
    data_type=enumordinal(TensorProto_DataType.INT64),
    int64_data = reshape(t,:),
    name=name)

TensorProto(t::AbstractArray{Int32,N}, name ="") where N = TensorProto(
    dims=collect(reverse(size(t))),
    data_type=enumordinal(TensorProto_DataType.INT32),
    int32_data = reshape(t,:),
    name=name)

TensorProto(t::AbstractArray{Int8,N}, name ="") where N = TensorProto(t, TensorProto_DataType.INT8, name)

TensorProto(t::AbstractArray, data_type::TensorProto_DataType.T, name) = TensorProto(
    dims=collect(reverse(size(t))),
    data_type=enumordinal(data_type),
