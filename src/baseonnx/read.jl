...
function array(p::TensorProto, wrap=Array)
    ...
    if p.data_type === var"TensorProto.DataType".INT32
    ...
    if p.data_type === var"TensorProto.DataType".INT8
        return reshape(reinterpret(Int8, p.raw_data), reverse(p.dims)...) |> wrap
    end

    if p.data_type === var"TensorProto.DataType".DOUBLE
    ...
    if p.data_type === var"TensorProto.DataType".FLOAT
    ...
    if p.data_type === var"TensorProto.DataType".FLOAT16
